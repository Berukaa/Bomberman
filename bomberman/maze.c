...
void loadMaze (char * filename, maze_t * maze)
{
	...
	for (line = 0, nbPlayer = 0; line < maze->h; line++)
	{
		...
		{
			// Coordonn�e lin�aire de la case courante.
			...
			switch (buffer[row])
			{
				...
				case '+':
					...
					// G�n�ration al�atoire des bonus.
				...
				case '@':
					...
					if (nbPlayerDefault == 0 || nbPlayer < nbPlayerDefault)
					{
						...
						arrayPlayer[nbPlayer].x = row; 					// Affectation des coordonn�s.
						arrayPlayer[nbPlayer].y = line; 					// Affectation des coordonn�s.
...
void updatePlayer (maze_t * maze, int stepByStep)
{
	...
	// Gestion des d�placements des joueurs.
	for (iPlayer = 0; iPlayer < nbPlayerDefault; iPlayer++)
	{
